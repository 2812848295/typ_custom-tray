...
function setOptions(options: { tray: Tray; window: BrowserWindow }) {
  if (!validation(options)) return;

  init(options);
}

function validation(options: any) {
...
function createTray(trayIconPath: any) {
  tray = new Tray(trayIconPath);
}

function setTray(newTray: any) {
  tray = newTray;
}

function setWindow(newWindow: any) {
  window = newWindow;
  setWindowSize(window.getBounds());
}

function createWindow(windowUrl: any) {
  window = undefined;
...
function setWindowUrl(windowUrl: any) {
...
export { setOptions, setTray, setWindow, setWindowSize };
